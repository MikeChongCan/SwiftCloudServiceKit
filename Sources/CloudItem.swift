...
public struct CloudItem: Hashable, Codable, Sendable {
    ...
    public init(id: String, name: String, path: String, isDirectory: Bool = true, json: [String: Any] = [:]) {
        ...
        self.json = json.mapValues { AnyCodable($0) }
    }
    ...
    public mutating func fixPath(with directory: CloudItem) {
        if directory.path == "/" {
            path = "/" + path
        } else {
...
public struct CloudUser: Codable, Sendable {
...
public struct CloudSpaceInformation: Codable, Sendable {
